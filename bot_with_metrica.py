...
async def send_to_metrika(user_id: int, event_url: str, event_title: str, event_params: dict = None):
    ...
    Не блокирует бота при ошибках
    """
    if not YA_METRIKA_COUNTER_ID or not YA_METRIKA_MP_TOKEN:
        return  # Метрика не настроена
    ...
    # Отправляем асинхронно, не блокируя бота
    try:
        import aiohttp
        async with aiohttp.ClientSession() as session:
            await session.get(full_url, timeout=aiohttp.ClientTimeout(total=3))
    except Exception as e:
        logger.warning(f"Метрика не ответила: {e}")

def sync_send_to_metrika(user_id: int, event_url: str, event_title: str, event_params: dict = None):
    """Синхронная версия для использования в не-async функциях"""
    if not YA_METRIKA_COUNTER_ID or not YA_METRIKA_MP_TOKEN:
        return
...
async def send_reminder(context):
    ...
    # ===== [АНАЛИТИКА] =====
    sync_send_to_metrika(task["assignee_id"], "/reminder_received", f"Напоминание о дедлайне", {"hours_before": data["hours"], "task_id": data["task_id"]})
    # =======================
...
async def delay_reminder_callback(update, context):
    ...
    # ===== [АНАЛИТИКА] =====
    await send_to_metrika(update.effective_user.id, "/reminder_snoozed", "Откладывание напоминания", {"hours": hours, "task_id": task_id})
    # =======================
...
async def postpone_reason(update, context):
    ...
    # ===== [АНАЛИТИКА] =====
    await send_to_metrika(update.effective_user.id, "/postpone_requested", "Запрос переноса дедлайна", {"task_id": task_id, "reason_length": len(reason)})
    # =======================
...
async def handle_time_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("calendar_next_state") == "task_creation":
        ...
        if assign_all:
            ...
            # ===== [АНАЛИТИКА] =====
            await send_to_metrika(update.effective_user.id, "/task_created_all", "Создание задачи для всех", {"project_id": project_id, "task_title": title[:50]})
            # =======================
        else:
            ...
            # ===== [АНАЛИТИКА] =====
            await send_to_metrika(update.effective_user.id, "/task_created_single", "Создание задачи", {"assignee_id": assignee_id, "task_title": title[:50]})
            # =======================
